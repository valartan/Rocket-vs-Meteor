 ...
 (() => {
  ...
  function resetGame() {
    bubbles = [];
    particles = []; // reset aussi les particules à chaque partie
    frameCount = 0;
    gameOver = false;
    distance = 0;
  ...
  function gameLoop(timestamp) {
    ...
    if (!gameOver) {
      for (let i = bubbles.length - 1; i >= 0; i--) {
        if (isColliding(player, bubbles[i])) {
          gameOver = true;
          createExplosion(player.x, player.y);  // Explosion au point d'impact
          bubbles.splice(i, 1);
          break;
        }
      }
    ...
    if (!gameOver || particles.length > 0) {
      // On continue tant que la partie n'est pas terminée OU que les particules explosent encore
      requestAnimationFrame(gameLoop);
    } else {
      displayLeaderboard();
