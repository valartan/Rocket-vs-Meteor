(() => {
  ...
  const player = {
    ...
    radius: 25,
    velocityY: 0,
    gravityUp: -0.8,    // montée moins forte
    gravityDown: 0.9,   // descente plus forte
    maxSpeed: 6,
  };
  ...
  const pulse = Math.sin(flamePulse) * 0.5 + 0.5; // oscille entre 0 et 1
  const flameLength = 30 + pulse * 25; // varie entre 30 et 55
  const flameWidth = 20 + pulse * 10;  // varie entre 20 et 30
  ...
  function drawMeteorite(b) {
    ...
    b.craters.forEach(c => {
      ctx.beginPath();
      ctx.arc(b.x + c.offsetX, b.y + c.offsetY, c.size, 0, Math.PI * 2);
      ctx.fill();
    });
  }
  ...
  function gameLoop(timestamp) {
    ...
    if (!gameOver) {
      // Gestion de la vélocité verticale avec gravité
      player.velocityY += pressing ? player.gravityDown : player.gravityUp;

      // Limites vitesse verticale
    ...
    frameCount++;
flamePulse += 0.15; // contrôle la vitesse du rythme de la poussée
